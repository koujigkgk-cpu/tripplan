...
// A. まず大元の変数を定義する
var outboundExpenses = { hotel: [], food: [], other: [], gas: 0, highway: 0 };
var returnExpenses = { hotel: [], food: [], other: [], gas: 0, highway: 0 };

// B. その後に「呪い対策」の代名詞を作る
var outboundExp = outboundExpenses;
var returnExp = returnExpenses;
...
function restoreWaypoints() {
    const items = document.querySelectorAll('#restore-data .wp-item');
    items.forEach(item => {
        const data = {
        ...
        // マーカーを立てる処理...
        // 入力フォームを作成し、値をセットする
        const inputDesc = document.querySelector('.some-desc-input'); // 実際クラス名に合わせてください
        if (inputDesc) inputDesc.value = data.description;

        const inputStay = document.querySelector('.some-stay-input');
...
function renderWaypointList() {
    ...
    if (container) {
        ...
        waypoints.forEach(function(wp) {
            ...
            // 【重要】地図上のピンを制御するロジック（これを消すとピンが残ります）
...
function calcTotal() {
    ...
    // 3. 宿泊費・食費・その他の合計を出す
    var currentExp = isReturnTrip ? returnExpenses : outboundExpenses;
...
function saveFullPlan() {
    ...
    getRoute().then(() => {
        ...
        if (isReturnTrip) {
            // 【復路】なら名前も座標も入れ替えて、DB上の「出発地」を東京にする
            finalOriginName = document.getElementById('dest-input').value;   // 目的地欄(東京)をDBの出発地に
            finalDestName = document.getElementById('origin-input').value; // 出発地欄(福岡)をDBの目的地に
            ...
            // 【往路】ならそのまま
            finalOriginName = document.getElementById('origin-input').value;
            finalDestName = document.getElementById('dest-input').value;
        ...
        var fullPlanData = {
            originName: finalOriginName, // 確定した名前
            destName: finalDestName,   // 確定した名前
            ...
            outbound: {
                fuelCost: outboundExpenses.gas || 0,
                highwayCost: outboundExpenses.highway || 0,
                ...
                expenses: outboundExpenses
            },

            return: {
                fuelCost: returnExpenses.gas || 0,
                highwayCost: returnExpenses.highway || 0,
        ...
        console.log("📝 送信データ詳細:", fullPlanData);
        ...
        fetch("SaveFullPlanServlet", {
            ...
            .then(res => res.ok ? alert("保存しました！") : alert("保存失敗"))
...
function saveExp() {
    ...
    if (isReturnTrip) {
        returnExpenses = summary;
    } else {
        outboundExpenses = summary;
    }
}
function reverseRoute() {
    ...
    if (!isReturnTrip) {
        ...
        restoreExpenses(returnExpenses);
        ...
        restoreExpenses(outboundExpenses);
    }
...
function updateStayTime(id, value) {
    ...
    document.querySelectorAll('input').forEach(function(el) {
        var attr = el.getAttribute('oninput');
        if (attr && attr.includes("updateStayTime('" + id + "'")) {
...
function calcGasCost() {
    ...
    if (isReturnTrip) {
        returnExpenses.gas = gasCost;
    } else {
        outboundExpenses.gas = gasCost;
    }
...
function calcHighwayCost() {
    ...
    if (isReturnTrip) {
        returnExpenses.highway = highwayCost;
    } else {
        outboundExpenses.highway = highwayCost;
    }
...
function updateGrandTotal() {
    // --- 往路の集計 ---
    let outboundHotel = 0, outboundFood = 0, outboundOther = 0;
    if (outboundExpenses && outboundExpenses.hotel) {
        outboundExpenses.hotel.forEach(item => outboundHotel += item.price || 0);
    }
    if (outboundExpenses && outboundExpenses.food) {
        outboundExpenses.food.forEach(item => outboundFood += item.price || 0);
    }
    if (outboundExpenses && outboundExpenses.other) {
        outboundExpenses.other.forEach(item => outboundOther += item.price || 0);
    }

    outboundExpenses.hotelTotal = outboundHotel;
    outboundExpenses.foodTotal = outboundFood;
    outboundExpenses.otherTotal = outboundOther;

    // --- 復路の集計 ---
    let returnHotel = 0, returnFood = 0, returnOther = 0;
    if (returnExpenses && returnExpenses.hotel) {
        returnExpenses.hotel.forEach(item => returnHotel += item.price || 0);
    }
    if (returnExpenses && returnExpenses.food) {
        returnExpenses.food.forEach(item => returnFood += item.price || 0);
    }
    if (returnExpenses && returnExpenses.other) {
        returnExpenses.other.forEach(item => returnOther += item.price || 0);
    }

    returnExpenses.hotelTotal = returnHotel;
    returnExpenses.foodTotal = returnFood;
    returnExpenses.otherTotal = returnOther;

    // --- 片道ごとの合計 ---
    const outboundTotal =
        (outboundExpenses.hotelTotal || 0) +
        (outboundExpenses.foodTotal || 0) +
        (outboundExpenses.otherTotal || 0) +
        (outboundExpenses.gas || 0) +
        (outboundExpenses.highway || 0);

    const returnTotal =
        (returnExpenses.hotelTotal || 0) +
        (returnExpenses.foodTotal || 0) +
        (returnExpenses.otherTotal || 0) +
        (returnExpenses.gas || 0) +
        (returnExpenses.highway || 0);

    // --- 往復合計 ---
    const totalAll = outboundTotal + returnTotal;
...
function forceUpdateCurrentModeTotal() {
    ...
    var targetExp = isReturnTrip ? returnExpenses : outboundExpenses;
...
function updateRoundTripGrandTotal() {
    ...
    try {
        // --- 1. 補助関数：画面の表示文字から数字だけを抜き出す ---
        const getValFromDisplay = (id) => {
            const el = document.getElementById(id);
            if (!el) return 0;
            // 「¥ 30,978」などの文字から数字以外を除去して数値化
            return parseInt(el.innerText.replace(/[^0-9]/g, "")) || 0;
        };

        // --- 2. 現在の編集内容を、リアルタイムに往路/復路の小計へ反映 ---
        // 黒背景の「リアルタイム合計予算」の数字を取得
        const currentEditTotal = getValFromDisplay('disp-total');

        // 今が「帰り」モードなら復路のラベルを、そうでなければ往路のラベルを即座に更新
        if (typeof isReturnTrip !== 'undefined' && isReturnTrip) {
            const retSub = document.getElementById('return-total-sub');
            if (retSub) retSub.innerText = "¥ " + currentEditTotal.toLocaleString();
        } else {
            const outSub = document.getElementById('outbound-total-sub');
            if (outSub) outSub.innerText = "¥ " + currentEditTotal.toLocaleString();
        }

        // --- 3. 往路ラベルと復路ラベルの数字を足して「本当の総合計」を出す ---
        // あなたが指差した「往路: ¥ ...」と「復路: ¥ ...」の場所を読み取ります
        const outVal = getValFromDisplay('outbound-total-sub');
        const retVal = getValFromDisplay('return-total-sub');
        const finalSum = outVal + retVal;

        // --- 4. 画面の一番上（往復総合計）を書き換え ---
        const gtEl = document.getElementById('grand-total-display');
        if (gtEl) {
            gtEl.innerText = "¥ " + finalSum.toLocaleString();
            // 反映されたことが分かりやすいように少し色を強調（任意）
            gtEl.style.color = "#d44000";
        }

        console.log("✅ 往路(" + outVal + ") + 復路(" + retVal + ") = 総合計(" + finalSum + ")");
...
// 1秒ごとにこの同期を繰り返す
//setInterval(updateRoundTripGrandTotal, 1000);
/**
 * 【画面完全再現】DBから届いたjspConfigを元に、
 * 保存時と同じ地図・リスト・予算を表示します。
 */
(function() {
    // ページ読み込み完了時に実行
...
function restoreExpenses(data) {
    ...
    ['hotel', 'food', 'other'].forEach(function(type) {
        ...
        // ★ データが「有効な配列」かつ「中身がある」場合のみそれを使う
        if (Array.isArray(data[type]) && data[type].length > 0) {
            items = data[type];
        } else {
            // ★ データが空、または数値だった場合はデフォルトの1行を作成
            items = [{
        ...
        if (isReturnTrip) {
            returnExpenses[type] = items;
        } else {
            outboundExpenses[type] = items;
        }
        calcTotal();
    });
}
...
// エラー回避用の空関数
/**
 * 入力された住所（テキスト）を、内部の座標やデータと同期させる関数
 */
function syncAddress() {
